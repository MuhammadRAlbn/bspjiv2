<x-layouts.app title="Pengujian - SIPINTU">
    ...
    <!-- Hero Section -->
    <header
        class="relative h-[400px] w-[96%] mx-auto mt-5 mb-10 md:mt-[20px] flex items-center text-white overflow-hidden rounded-[25px]">
        <img src="https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80&w=2070"
            alt="Pengujian Hero" class="absolute inset-0 w-full h-full object-cover -z-10 brightness-[0.7]">
    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col gap-3 lg:sticky lg:top-24">
            ...
            @foreach($tabs as $tab)
                <button @click="setTab('{{ $tab['id'] }}')" :class="activeTab === '{{ $tab['id'] }}' 
                    ...
                    <i class="fas {{ $tab['icon'] }} text-[20px]"
                        :class="activeTab === '{{ $tab['id'] }}' ? 'text-white' : 'text-slate-500'"></i>
        ...
        <article x-ref="contentArea" class="pb-[150px] min-h-[70vh]">
            ...
            <section x-show="activeTab === 'sertifikasi'" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0"
                class="animate-fade-in">
                @include('layanan.pengujian.partials.sertifikasi', ['sertifikasi' => $sertifikasi])
            </section>

            <!-- Tab: Ruang Lingkup (Blade Partial) -->
            <section x-show="activeTab === 'ruang-lingkup'" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                @include('layanan.pengujian.partials.ruang_lingkup', ['laboratorium' => $laboratorium, 'ruangLingkup' => $ruangLingkup])
            </section>

            <!-- Tab: Alur (Blade Partial) -->
            <section x-show="activeTab === 'alur'" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                @include('layanan.pengujian.partials.alur', ['alur' => $alur])
            </section>

            <!-- Tab: Tarif (Livewire Component) -->
            <section x-show="activeTab === 'tarif'" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
